          ...
          {chainId && (
            <ExternalLink style={{ fontWeight: 400 }} href={getBscScanLink(chainId, token.address, 'token')}>
              <Text title={token.address}>
                {shortenAddress(token.address)} {TranslateString(116, '(View on Moonbeam Explorer)')}
              </Text>
            </ExternalLink>
          )}
        </AutoColumn>
      ...
      <WarningContainer className="token-warning-container">
        <AutoColumn gap="lg">
          ...
          <Text>
            {TranslateString(
              1130,
              'Anyone can create a token on Moonbeam with any name, including creating fake versions of existing tokens and tokens that claim to represent projects that do not have a token.'
            )}
          </Text>
          <Text>
            {TranslateString(
